...
----------------------------------------------------------------------------------------------------
 ...
 function HQ:OnReset() -- Remod HQ HP 1
        ...
        self.destroyed=false;	
	-- A TAC hit actually deals 6000 points of damage. However, by using 5999 we prevent any tiresome errors
	-- where the HQ does not actually die.
	local health = 3; -- Number of hits it takes to destroy
	Log("HQ HP set to %q", health);
...
function HQ.Server:OnHit(hit) -- Remod HQHP 2
        ...
        if (teamId==0 or teamId~=self:GetTeamId()) then
			...
			if(not destroyed) then
                      		 	...
                      		 	Log(team.. " HQ now has "..tostring(damage).."% damage");
