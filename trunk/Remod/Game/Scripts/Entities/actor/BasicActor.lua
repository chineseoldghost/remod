...
{	
	...
	gameParams =
	{
		...
		sprintMultiplier = 1.65, -- Speed is multiplied by this amount if sprint key is pressed -- 1.2 for a more counter-striky feel
		...
		afterburnerMultiplier = 5.0, -- How much the afterburner
		
		-- Grabbing
		maxGrabMass = 70,
		maxGrabVolume = 2.0, -- It's the square volume of an 1x1x2 meters object
	},
}
...
function BasicActor:ShutDown()  
	...
	--self:ResetAttachment(0, "laser_attachment"); -- Remod
...
function BasicActor:UpdateSounds(frameTime)
	...
	if (newStance ~= oldStance) then
		...
		elseif (newStance == STANCE_PRONE) then
			stanceSound = "sounds/physics:player_foley:prone_on"; -- Remod, default is prone_on
		else
			--standup
