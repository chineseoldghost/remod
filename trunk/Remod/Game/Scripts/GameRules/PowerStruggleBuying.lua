...
--	{ id="moar",			name="@mp_eAlienMOAR",			price=100, 		class="MOARAttach", 			level=50,		uniqueId=12,	category="@mp_catWeapons", loadout=1 },
	...
	{ id="taclauncher",					name="@mp_eTACLauncher", 			price=500, 		class="TACLauncher", 					level=100,	energy=25, uniqueId=14,	category="@mp_catWeapons", md=true, loadout=1, uniqueloadoutgroup=1, uniqueloadoutcount=2},
...
--	{ id="nktactank",			name="@mp_eLightTACTank",					price=500,		class="Asian_tank", 					level=100, 	energy=10, modification="TACCannon",		vehicle=true, md=true, buildtime=45,	category="@mp_catVehicles", loadout=0 },
	...
	{ id="gauss",						name="@mp_eGauss", 							price=250, 			class="GaussRifle",		level=50,		uniqueId=10,	category="@mp_catWeapons", loadout=1, 		uniqueloadoutgroup=1, uniqueloadoutcount=2},
}

PowerStruggle.vehicleList=
{	
	{ id="light4wd",				name="@mp_eLightVehicle", 				price=25,			class="US_ltv",						modification="Unarmed", 		buildtime=5,		category="@mp_catVehicles", loadout=0 },
...
--	{ id="nkaaa",					name="@mp_eAAVehicle",					price=200,		class="Asian_aaa", 				modification="MP",	buildtime=15,			category="@mp_catVehicles", loadout=0 },
	...
	{ id="usasv",					name="@mp_eASV",							price=250,		class="US_asv",						modification="AVMachinegun", buildtime=15,		category="@mp_catVehicles", loadout=0 },
	...
	{ id="usvtol",					name="@mp_eVTOL", 						price=250,		class="US_vtol", 					modification="MP", buyzoneradius=11, servicezoneradius=11,	buyzoneflags=bor(bor(PowerStruggle.BUY_AMMO, PowerStruggle.BUY_WEAPON), PowerStruggle.BUY_EQUIPMENT),	buildtime=30,		category="@mp_catVehicles", loadout=0 },	
};
...
PowerStruggle.ammoList=
{
	...
	{ id="bullet",							name="@mp_eBullet", 					price=5,			amount=24,				category="@mp_catAmmo", loadout=1 },
	...
	{ id="scargrenade",					name="@mp_eRifleGrenade",			price=35,			amount=1,					category="@mp_catAmmo", loadout=1 },
	...
	{ id="hurricanebullet",			name="@mp_eMinigunBullet",		price=100,			amount=250,				category="@mp_catAmmo", loadout=1 },
	
	{ id="fgl40fraggrenade",		name="@mp_eFGL40FragGrenade",		price=100,			amount=6,				category="@mp_catAmmo", loadout=1 },
	{ id="fgl40empgrenade",		name="@mp_eFGL40EMPGrenade",			price=100,			amount=6,				category="@mp_catAmmo", loadout=1 },

	{ id="claymoreexplosive",																price=75,			amount=1,			invisible=true,		category="@mp_catAmmo", loadout=1 },
	{ id="avexplosive",																			price=50,			amount=1,			invisible=true,		category="@mp_catAmmo", loadout=1 },
	{ id="c4explosive",																			price=125,		amount=1,			invisible=true,		category="@mp_catAmmo", loadout=1 },
...
--[[function PowerStruggle:PowerTick()
        ...
        if (self.powerTick<=0) then
                ...
                if (self.powerpoints) then
                        local capturedSites = {NK=0; US=0; TOT=#self.powerpoints;}
                        local teamIds = {NK=nil; US=nil;}
                        for i,teamId in ipairs(self.teamId) do
                                local sum=0;
                                for j,id in ipairs(self.powerpoints) do
                                        local powerTeamId=self.game:GetTeam(id);
                                        if (teamId==powerTeamId) then
                                            local n = g_gameRules.game:GetTeamName(teamId);
                                            local teamname = string.upper(n);
                                            capturedSites[teamname] = capturedSites[teamname] + 1;
                ...
                for team,sites in pairs(capturedSites) do
                           if (sites==0 or sites=="0") then
                                teamId=teamIds.team;
                                self:SetTeamPower(self:GetTeamPower(teamId)-(1*scale));     
                           end
                end
                                self:SetTeamPower(teamId, self:GetTeamPower(teamId)+sum*scale);
            end
...
----------------------------------------------------------------------------------------------------
--function SetupTacGarbage() -- Remod
--       local ps = g_gameRules;
--       local bl = ps.buyList;
--
--	bl.tacgun = {
--		id="tacgun",
--		name="@mp_eTACLauncher", 
--		price=500, 
--		class="TACGun", 
--		level=100,
--		restricted=1,	
--		uniqueId=14,			 
--		md=true,
--		category="@mp_catWeapons", 
--		loadout=1, 		
--		uniqueloadoutgroup=1,
--		uniqueloadoutcount=2
--	};
--
--        for i,fac in ipairs(g_gameRules.factories) do
--        	local kv = fac.weapons;
--		if (kv.moar) then
--			kv.tacgun = true;
--		end
--	end
